...
class PatchConvnet(nn.Module):
    def __init__(
        ...
        if not self.multiclass:
            self.head = nn.Linear(in_features=int(embed_dim), out_features=int(self.last_embed_dim)) if num_classes > 0 else nn.Identity()
        else:
...
@register_model
def S60(emb_size: int, pretrained: bool = False, **kwargs):
    model = PatchConvnet(
        patch_size=16,
        embed_dim=384,
        num_classes=1,
        last_embed_dim=emb_size,
        depth=60,
        num_heads=1,
        qkv_bias=True,
