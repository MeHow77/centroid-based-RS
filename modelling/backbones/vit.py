...
class vit_models(nn.Module):
    ...
    def load_param(self, model_path):
        if not model_path:
            name = "https://dl.fbaipublicfiles.com/deit/deit_3_small_224_21k.pth"
...
@register_model
def deit_tiny_patch16_LS(pretrained=False, pretrained_21k = False,   **kwargs):
    model = vit_models(
        default_img_size=224, new_img_size=320, patch_size=16, embed_dim=192, depth=12, num_heads=3, mlp_ratio=4, qkv_bias=True,
...
@register_model
def deit_small_patch16_LS(pretrained=False, pretrained_21k = False, last_emb_size=1024, img_size=320, **kwargs):
    model = vit_models(
        img_size=img_size, patch_size=16, embed_dim=384, num_classes=last_emb_size, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),block_layers=Layer_scale_init_Block, **kwargs)
    return model
